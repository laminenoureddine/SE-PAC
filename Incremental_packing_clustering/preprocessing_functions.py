...
def parse_special_line(line):
    ...
    if line.startswith('#Short_feature_name'):
        global_vars.feature_names_short = list(map(str.rstrip, line.split(',')[1:]))
    elif line.startswith('#Long_feature_name'):
        global_vars.feature_names = list(map(str.rstrip, line.split(',')[1:]))
    elif line.startswith('#Features_category'):
        global_vars.features_categories = list(map(str.rstrip, line.split(',')[1:]))
    ...
    for file in global_vars.file_names:
        ...
        if not isfile(filename):
            ...
            for featline in loaded_features_array:
                if True in list(map(isinf, featline)):
                    log('ERROR: Infinity in features of file ' + filename)
                if True in list(map(isnan, featline)):
...
def load_mnemonics_features_dir(clustering_mode, feat_dir):
    for file in global_vars.file_names:
        ...
        if not isfile(filename):
            ...
            fam = file
            if fam in global_vars.features_mnemonics_dict:
                global_vars.features_mnemonics_dict[fam] += load_mnemonics_feature_file(filename)
            else:
                global_vars.features_mnemonics_dict[fam] = load_mnemonics_feature_file(filename)
        ...
        if clustering_mode == "no_consensus_only":
            ...
            if not isfile(non_cons_filename):
                ...
                fam = file
                if fam in global_vars.features_mnemonics_dict:
                    global_vars.features_mnemonics_dict[fam] += load_mnemonics_feature_file(non_cons_filename)
                else:
                    global_vars.features_mnemonics_dict[fam] = load_mnemonics_feature_file(non_cons_filename)
...
def fam_to_remove(min_samples_per_family):
    ...
    for missingfam in toremove:
        ...
        global_vars.features_mnemonics_dict.pop(missingfam, None)
...
def assign_labels_and_features():
    ...
    for i, fam in enumerate(global_vars.family_names):
        ...
        if (first_concatenation):
            ...
            global_vars.features_mnemonics = global_vars.features_mnemonics_dict[fam]
