...
def main_process(feat_dir,
    ...
    with open('hardcoded_data/feature_types.txt') as f:
        global_vars.feature_types = [line.rstrip('\n') for line in f]
    f.close()

    with open('hardcoded_data/feature_types_range.txt') as f:
        global_vars.feature_types_range = [line.rstrip('\n') for line in f]
    f.close()

    with open('hardcoded_data/features_description_with_comments.txt') as f:
    ...
    # get the amount of samples in each family: needed it later
    ...
    table_field_names = ["feature_number", "feature_name", "feature_importance_rank", "type", "type_range",
                         "global_range"]
    table_field_names.extend(global_vars.family_names)
    ...
    for i in range(0, len(global_vars.features[0])):
        ...
        for item in unique_fam_amout.items():
            ...
            min = preprocessing_functions.drop_traling_zeros(float(min))
            max = format(max, format_string)
            max = preprocessing_functions.drop_traling_zeros(float(max))
            fam_range_values[fam] = '[' + str(min) + ',' + str(max) + ']'
        ...
        row = [(i + 1), feature_names_short_i, feature_importance_desc_rank_i, features_type_i, features_type_range_i,
               global_range_i]
        row.extend(list_fam_range_values)
        row.append(features_desc_and_comments_i)
        features_fam_table.add_row(row)

    preprocessing_functions.log("\n\nComparative table: features range between families")
